...
class Debug
{
	...
	protected static function _dump( & $var, $length = 128, $limit = 10, $level = 0)
	{
		...
		elseif (is_resource($var))
		{
			if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
			{
				...
				if (isset($meta['uri']))
				{
					...
					return '<small>resource</small><span>('.$type.')</span> '.htmlspecialchars($file, ENT_NOQUOTES, Yi::$charset);
				}
			}
			else
			{
		...
		{
			// Encode the string
			$str = htmlspecialchars($var, ENT_NOQUOTES, Yi::$charset);
		...
		elseif (is_array($var))
		{
			...
			elseif ($level < $limit)
			{
				...
				foreach ($var as $key => & $val)
				{
					...
					if ( ! is_int($key))
					{
						$key = '"'.htmlspecialchars($key, ENT_NOQUOTES, Yi::$charset).'"';
					}
		...
		{
			return '<small>'.gettype($var).'</small> '.htmlspecialchars(print_r($var, TRUE), ENT_NOQUOTES, Yi::$charset);
		}
	}
	...
	public static function source($file, $line_number, $padding = 5)
	{
		...
		while (($row = fgets($file)) !== FALSE)
		{
			...
			if ($line >= $range['start'])
			{
				...
				$row = htmlspecialchars($row, ENT_NOQUOTES, Yi::$charset);
